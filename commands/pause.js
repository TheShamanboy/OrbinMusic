...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (botVoiceChannel && interaction.member.voice.channel.id !== botVoiceChannel.id) {
            return interaction.reply('You need to be in the same voice channel as me to pause.');
        }
        ...
        if (player.isPlaying()) {
            ...
            return interaction.reply('The music is already paused. Use `/resume` to resume playback.');
        }
    },
    ...
    async executeMessage(context) {
        ...
        if (botVoiceChannel && context.member.voice.channel.id !== botVoiceChannel.id) {
            return context.reply('You need to be in the same voice channel as me to pause.');
        }
        
        // Pause the player
        if (player.isPlaying()) {
            player.pause();
            return context.reply('⏸️ Paused the music.');
        } else {
            return context.reply('The music is already paused. Use `+resume` to resume playback.');
        }
    },
